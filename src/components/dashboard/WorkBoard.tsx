...
const WorkBoard = ({ isOverlay, onMinimize }: WorkBoardProps) => {
  ...
  const connectGmail = async () => {
    ...
    try {
      const { error } = await supabase.auth.signInWithOAuth({
        provider: "google",
        options: {
          redirectTo: "https://geoterrainfo.com/auth/callback",
      ...
      if (error) {
        ...
        toast({
          title: "Greška",
          description: "Ne mogu pokrenuti Gmail povezivanje.",
          variant: "destructive",
        });
      }
    } catch (error) {
      console.error("Unexpected Gmail connect error:", error);
      toast({
        title: "Greška",
        description: "Došlo je do neočekivane greške kod Gmail povezivanja.",
        variant: "destructive",
      });
    } finally {
  ...
  const togglePrivacy = async (item: WorkspaceItem) => {
    ...
    toast({
      title: item.is_private ? "Premješteno u javno" : "Premješteno u privatno",
    });
  };
          ...
          {isEditing ? (
            ...
            >
              ...
              <div className="flex gap-1 justify-end">
                <Button type="button" variant="ghost" size="sm" className="h-6 text-xs" onClick={() => setEditingItemId(null)}>
                  Odustani
                </Button>
        ...
        <div className="space-y-1 mt-1.5">
          {archivedList.length === 0 ? (
            ...
            archivedList.map((item) => (
              <div
                key={item.id}
                className="flex items-center gap-2 p-1.5 rounded-lg border bg-muted/20 border-border/50 group"
              >
  ...
  const content = (
    <>
                    ...
                    <TooltipTrigger asChild>
                      <button
                        onClick={() => {
                          ...
                          if (!isOwnProfile) fetchOtherUserPrivateItems(profile.user_id);
                        }}
                        className={cn(
      ...
      <CardContent className="px-3 pb-3 pt-0 flex flex-col gap-2 flex-1 min-h-0 overflow-y-auto scrollbar-hide">
        ...
        <div className="border-t border-border/50 pt-2 mt-1">
          <div className="flex items-center gap-1.5 mb-1.5">
            ...
            <a
              href="https://mail.google.com/mail/u/?authuser=geoterra@geoterrainfo.net"
              target="_blank"
          ...
          {emails.map((email) => (
            <div
              key={email.id}
              className="flex items-start gap-1.5 py-1 px-1 rounded text-[11px] text-muted-foreground hover:bg-muted/30 transition-colors cursor-default"
              title={`${email.from}\n${email.subject}\n${email.snippet}`}
            >
      ...
      <Dialog
        open={saveDialogOpen}
        onOpenChange={(o) => {
          if (!o) setSaveDialogOpen(false);
        }}
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Spremi u Poslove</DialogTitle>
          </DialogHeader>
          ...
          <div className="space-y-3">
            <div>
              ...
              <Select value={selectedBoard} onValueChange={handleBoardChange}>
                <SelectTrigger>
                  <SelectValue placeholder="Odaberi board..." />
                </SelectTrigger>
                <SelectContent>
                  {boards.map((b) => (
            ...
            {selectedBoard && (
              <div>
                ...
                <Select value={selectedColumn} onValueChange={handleColumnChange}>
                  <SelectTrigger>
                    <SelectValue placeholder="Odaberi kolonu..." />
                  </SelectTrigger>
                  <SelectContent>
                    {columns.map((c) => (
            ...
            <Button onClick={handleSaveToKanban} disabled={!selectedCard} className="w-full">
              Spremi na karticu
            </Button>
          </div>
        </DialogContent>
        ...
        (() => {
          ...
          return (
            <div
              ...
              onClick={() => setPrivateOverlayOpen(false)}
            >
              <Card
                className="w-full max-w-6xl mx-4 max-h-[95vh] min-h-[70vh] flex flex-col shadow-2xl border-primary/20"
