...
const AgentStatusBadge = () => {
  ...
  return (
    ...
    >
      ...
      />
      Agent{" "}
      {agentOk === true ? "online" : agentOk === false ? "offline" : "..."}
    </div>
  );
};
...
const ChatDialog = ({
  ...
  const [budgetEur, setBudgetEur] = useState<string>(
    () => localStorage.getItem("stellan_budget") || "",
  );
  const [spentEur, setSpentEur] = useState<number>(() =>
    parseFloat(localStorage.getItem("stellan_spent") || "0"),
  );
  const [savedActions, setSavedActions] = useState<
    { name: string; file: string }[]
  >([]);
  const [recordedSteps, setRecordedSteps] = useState<
    { n: number; url: string; desc: string; screenshot?: string }[]
  >([]);
  ...
  const [studioRightTab, setStudioRightTab] = useState<
    "steps" | "console" | "actions" | "code"
  >("steps");
  const [studioSidebarTool, setStudioSidebarTool] = useState("playwright");
  const [studioInput, setStudioInput] = useState("");
  const [consoleLogs, setConsoleLogs] = useState<{ t: string; msg: string }[]>([
    { t: "dim", msg: "Dev Studio spreman" },
  ]);
  const [agentOnline, setAgentOnline] = useState<boolean | null>(null);
  const [projectRootState, setProjectRootState] = useState<string>(() =>
    typeof window !== "undefined"
      ? localStorage.getItem("stellan_project_root") || ""
      : "",
  );
  ...
  const [pendingImages, setPendingImages] = useState<
    { name: string; base64: string; size: number }[]
  >([]);
  ...
  const addDevStep = (action: string, label: string, target?: string) => {
    ...
    setDevSteps((prev) => [
      ...prev,
      { id, action, label, status: "running", target },
    ]);
    return id;
  };
  ...
  const devPanelSteps = devSteps.map((step) => ({
    id: step.id,
    action:
      (step.action as "open" | "click" | "type" | "screenshot" | "learn") ||
      "open",
    label: step.label,
    status: step.status,
    detail: step.detail,
    ...
    if (isListeningTask) {
      ...
      if (text && user) {
        ...
        setMessages((prev) => [
          ...
          {
            role: "assistant",
            content: `✅ Zadatak dodan na radnu ploču: **"${text}"**`,
          },
        ]);
      }
      return;
      ...
      for (const file of filesToProcess) {
        ...
        if (file.size > maxSize) {
          setMessages((prev) => [
            ...
            {
              role: "assistant",
              content: `❌ **${file.name}** je prevelika (max 20MB).`,
            },
          ]);
          continue;
        }
        ...
        if (isImage) {
          ...
          reader.onload = () => {
            ...
            setPendingImages((prev) => [
              ...prev,
              { name: file.name, base64, size: file.size },
            ]);
          };
          ...
          try {
            ...
            setPendingFiles((prev) => [
              ...prev,
              {
                ...
                pdfText:
                  fullText.trim() ||
                  "_Nema tekstualnog sadržaja (skenirani dokument)_",
            ...
            setPendingFiles((prev) => [
              ...prev,
              { name: file.name, size: file.size, type: "application/pdf" },
            ]);
          }
          continue;
        }
        ...
        const isTextByExt = textExtensions.some((ext) =>
  ...
  useEffect(() => {
    if (thinkingStatus) addLog("info", thinkingStatus);
  }, [thinkingStatus]);
  ...
  useEffect(() => {
    ...
    if (last?.role === "assistant" && last.content) {
      ...
      if (b64idx > -1) {
        ...
        if (b64end > -1) {
          setPreviewScreenshot(last.content.slice(b64idx, b64end));
          addLog("ok", "screenshot");
        }
      }
    }
  }, [messages.length]);
  ...
  const send = async () => {
    ...
    if (!rawText && pendingImages.length === 0 && pendingFiles.length === 0)
      return;
    ...
    if (
      ...
      rawText &&
      isLikelyProjectCommand(rawText)
    ) {
    ...
    if (
      ...
      const isAgentCommand =
        hasUrl ||
        /^(otvori|idi na|klikni|pritisni|upiši|upisi|unesi|čekaj|cekaj|screenshot|snimi|snimku)/i.test(
          lower,
        ) ||
        lower.includes("otvori ") ||
        lower.includes("idi na ") ||
        lower.includes("klikni ") ||
        lower.includes("pritisni ") ||
        lower.includes("screenshot") ||
        lower.includes("izvuci tekst");
    ...
    for (const f of pendingFiles) {
      ...
      if (f.pdfText) {
        ...
        parts.push(
          `\u00ab\u00abFILE:pdf:${f.name}:${sizeStr}:${pages}${urlPart}\u00bb\u00bb\n${f.pdfText}\n\u00ab\u00ab/FILE\u00bb\u00bb`,
        );
      } else if (f.content && f.language) {
        parts.push(
          `\u00ab\u00abFILE:${f.language}:${f.name}:${sizeStr}\u00bb\u00bb\n${f.content}\n\u00ab\u00ab/FILE\u00bb\u00bb`,
        );
      } else {
        parts.push(
          `\u00ab\u00abFILE:bin:${f.name}:${sizeStr}\u00bb\u00bb\n[binarni sadržaj]\n\u00ab\u00ab/FILE\u00bb\u00bb`,
        );
      }
    }

    // Pending images
    const imagesMd = pendingImages
    ...
    try {
      ...
      if (!currentSession?.access_token) {
        ...
        setMessages((prev) => [
          ...
          {
            role: "assistant",
            content: "⚠️ Sesija je istekla. Osvježi stranicu i pokušaj ponovo.",
          },
        ]);
        setIsLoading(false);
      ...
      if (!resp.body) {
        ...
        setMessages((prev) => [
          ...prev,
          { role: "assistant", content: "Nema odgovora od servera." },
        ]);
        if (streamTimeoutId) clearTimeout(streamTimeoutId);
        abortControllerRef.current = null;
        isStreamingRef.current = false;
    ...
    if (assistantSoFar) {
      const inputChars = newMessages.reduce(
      ...
      const inRate = isReasoning ? 2.0 : 0.2;
      const outRate = isReasoning ? 6.0 : 0.5;
      const costUsd =
  ...
  const callDevControl = useCallback(
    async (action: string, extra: Record<string, unknown> = {}) => {
      ...
      if (action !== "status" && !projectRoot) {
        throw new Error("Project root nije postavljen.");
      }
      ...
      if (!res.ok || data?.success === false) {
        throw new Error(
          data?.error || `DEV akcija nije uspjela (${res.status})`,
        );
      }
  ...
  const isLikelyProjectCommand = (raw: string) => {
    ...
    return (
      ...
      /^(postavi projekt root|postavi project root|set project root|root)\s+/i.test(
        text,
      )
    );
  };
  ...
  const executeProjectCommand = async (rawInput: string) => {
    ...
    const cleanQuoted = (value: string) =>
      value.trim().replace(/^["'`]|["'`]$/g, "");
    const getProjectRoot = () =>
      (localStorage.getItem("stellan_project_root") || "").trim();
    ...
    const likelyFileQuery = (value: string) => {
      ...
      if (
        /^(package|vite|tsconfig|vercel|tailwind|postcss|eslint|prettier|components)\b/i.test(
          q,
        )
      )
        return true;
    ...
    let match = raw.match(
      /^(?:postavi projekt root|postavi project root|set project root|root)\s+([\s\S]+)$/i,
    );
    if (match) {
    ...
    if (match) {
      ...
      if (result?.success) {
        ...
        pushAssistantMessage(
          `❌ Ne mogu pročitati file **${filePath}**: ${result?.error || "agent nedostupan"}`,
        );
    ...
    match = raw.match(
      /^(?:spremi file|zapisi file|zapiši file|write file)\s+([^\n]+)$/i,
    );
    if (match) {
      const filePath = cleanQuoted(match[1]);
      ...
      if (!content) {
        pushAssistantMessage(
          "⚠️ Za spremanje filea pošalji naredbu ovako:\n\n`spremi file src/.../Example.tsx`\n\npa ispod stavi full content u code blocku.",
        );
        return true;
      }
      const result = await callAgentDirect("write_file", {
        path: filePath,
        content,
        backup_first: true,
      });
      if (result?.success) {
        pushAssistantMessage(
          `✅ Spremljeno u \`${filePath}\`${result?.backup_path ? `\n\nBackup: \`${result.backup_path}\`` : ""}`,
        );
        addLog("ok", `Write file: ${filePath}`);
      } else {
        pushAssistantMessage(
          `❌ Ne mogu spremiti file **${filePath}**: ${result?.error || "agent nedostupan"}`,
        );
    ...
    match =
      ...
      raw.match(
        /^(?:pretrazi projekt|pretraži projekt|search project|trazi u projektu|traži u projektu)\s+(.+)$/i,
      );
    ...
    if (match) {
      ...
      if (likelyFileQuery(query)) {
        ...
        if (findResult?.success) {
          const files = Array.isArray(findResult.files)
            ? findResult.files.slice(0, 20)
            : [];
          if (!files.length) {
            pushAssistantMessage(
              `🔎 Nisam našao file **${query}** u projektu.`,
            );
          } else {
            const lines = files
              .map(
                (item: any) =>
                  `- \`${item.path}\`${item.size ? ` — ${item.size} B` : ""}`,
              )
              .join("\n");
            pushAssistantMessage(
              `### Find file\n\n**Pattern:** \`${query}\`\n**Root:** \`${projectRoot}\`\n\n${lines}`,
      ...
      if (result?.success) {
        ...
        if (!matches.length) {
          pushAssistantMessage(
            `🔎 Nisam našao ništa za **${query}** u projektu.`,
          );
        } else {
          const lines = matches
            .map(
              (item: any) =>
                `- \`${item.path}${item.line ? `:${item.line}` : ""}\` — ${String(item.text || "").trim()}`,
            )
            .join("\n");
          pushAssistantMessage(
            `### Search project\n\n**Query:** \`${query}\`\n**Root:** \`${projectRoot}\`\n\n${lines}`,
        ...
        pushAssistantMessage(
          `❌ Pretraga projekta nije uspjela: ${result?.error || "agent nedostupan"}`,
        );
    ...
    if (/^(?:pokreni build|run build|build)$/i.test(raw)) {
      ...
      const logHints = [result?.stdout_log, result?.stderr_log]
        .filter(Boolean)
      ...
      if (result?.success) {
        ...
        addLog(
          "warn",
          `Build failed${result?.error ? `: ${result.error}` : ""}`,
        );
      }
      return true;
    }
    ...
    if (/^(?:git status|status)$/i.test(raw)) {
      ...
      const result = await callAgentDirect("git_status", {
      ...
      if (result?.success) {
        ...
        pushAssistantMessage(
          `❌ Git status nije uspio.\n\n${makeCodeFence("git-status.txt", output)}`,
        );
        addLog(
          "warn",
          `Git status failed${result?.error ? `: ${result.error}` : ""}`,
        );
      }
      return true;
    }
    ...
    if (match) {
      ...
      if (!message) {
        pushAssistantMessage(
          '⚠️ Pošalji commit poruku. Primjer:\n\n`git commit "brain panel fix"`',
        );
        addLog("warn", "Git commit bez poruke");
        return true;
      }
      const result = await callAgentDirect("git_commit", {
        repo_path: projectRoot,
        message,
      });
      ...
      if (result?.success) {
        pushAssistantMessage(
          `✅ Git commit je prošao.\n\n**Poruka:** \`${message}\`\n\n${makeCodeFence("git-commit.txt", output)}`,
        );
        addLog("ok", `Git commit: ${message}`);
      } else {
        pushAssistantMessage(
          `❌ Git commit nije uspio.\n\n${makeCodeFence("git-commit.txt", output)}`,
        );
        addLog(
          "warn",
          `Git commit failed${result?.error ? `: ${result.error}` : ""}`,
        );
      }
      return true;
    }
    ...
    if (match) {
      ...
      if (result?.success) {
        ...
        addLog(
          "warn",
          `Git push failed${result?.error ? `: ${result.error}` : ""}`,
        );
      }
      return true;
    }
    ...
    if (/^(?:primijeni patch|primjeni patch|apply patch)\b/i.test(raw)) {
      ...
      if (!projectRoot) {
        pushAssistantMessage(
          "⚠️ Prvo postavi projekt root prije apply patcha.",
        );
      ...
      const afterCommand = raw
        .replace(/^(?:primijeni patch|primjeni patch|apply patch)\b/i, "")
        .trim();
      ...
      if (!parsedPayload) {
        pushAssistantMessage(
          "⚠️ Nisam uspio pročitati patch JSON. Pošalji `apply patch` i ispod JSON u code blocku.",
        );
...
const describeCurrentPreview = useCallback(
  ...
  const refreshPreviewScreenshot = useCallback(
    async (options?: { fullPage?: boolean; silent?: boolean }) => {
      const refreshed = await callAgentDirect("playwright", {
        action: "screenshot",
      ...
      if (refreshed?.success) {
        ...
        pushAssistantMessage(
          `⚠️ Ne mogu osvježiti preview: ${refreshed?.error || "agent nedostupan"}`,
        );
      }
      return refreshed;
    },
  ...
  const waitForPreviewReady = useCallback(
    async (timeoutMs = 5000) => {
      const waited = await callAgentDirect("playwright", {
        action: "wait",
        timeout: timeoutMs,
      });
      if (waited?.success) {
        ...
        addLog(
          "warn",
          "Čekanje nije uspjelo: " + (waited?.error || "agent nedostupan"),
        );
      }
      return waited;
    },
  ...
  const runPlaywrightAction = useCallback(
    async (
      payload: any,
      options?: {
      ...
      try {
        ...
        if (!data?.success) {
          ...
          pushAssistantMessage(
            `❌ Agent greška: ${data?.error || "nepoznata greška"}`,
          );
          return null;
        }
        ...
        if (options?.appendSummary && data.message) {
          pushAssistantMessage(
            `${options.summaryPrefix || "✅"} ${data.message}`,
          );
        }
        ...
        if (options?.describeAfter) {
          await describeCurrentPreview({
            heading: options.describeHeading || "### Stellan vidi u previewu",
            fallback:
              "Akcija je izvršena, ali nisam izvukao dovoljno teksta za sažetak.",
          });
        }
        return data;
      } finally {
  ...
  const executeStudioCommand = useCallback(
    async (cmd: string) => {
      const raw = cmd.trim();
      if (!raw) return;
      ...
      // NAVIGATE
      if (
        (lower.startsWith("idi na ") ||
          lower.startsWith("otvori ") ||
          lower.includes("otvori") ||
          lower.includes("idi na")) &&
        urlMatch
      ) {
        const stepId = addDevStep("open", `Open ${urlMatch[0]}`, urlMatch[0]);
        const nav = await runPlaywrightAction(
          { action: "navigate", url: urlMatch[0], timeout: 45000 },
          { appendSummary: true, summaryPrefix: "🌐" },
        );
        if (nav?.success) {
          updateDevStep(stepId, "done", nav.title || urlMatch[0]);
          const titleLine = nav.title ? `**${nav.title}**` : urlMatch[0];
          pushAssistantMessage(
            `🌐 Otvorio sam ${titleLine}. Screenshot je u previewu.`,
          );
        } else {
          updateDevStep(
            stepId,
            "error",
            nav?.error || "Navigacija nije uspjela",
          );
          pushAssistantMessage(
            `❌ Ne mogu otvoriti ${urlMatch[0]}: ${nav?.error || "greška"}`,
          );
        }
        return;
      }
      ...
      if (
        lower.includes("screenshot") ||
        lower.includes("snimku") ||
        lower.includes("snimi") ||
        lower.includes("što vidiš") ||
        lower.includes("sto vidis")
      ) {
        const stepId = addDevStep("screenshot", "Screenshot", previewUrl);
        const shot = await runPlaywrightAction(
        ...
        if (shot?.success) {
          ...
          updateDevStep(
            stepId,
            "error",
            shot?.error || "Screenshot nije uspio",
          );
        }
        return;
      }
      ...
      if (clickMatch) {
        ...
        if (data?.success) {
          ...
          pushAssistantMessage(
            `❌ Klik na **${target}** nije uspio: ${data?.error || "greška"}`,
          );
        }
        return;
      }
      ...
      // FILL / TYPE
      const fillQuoted = raw.match(
        /^(upiši|upisi|unesi|unesite)\s+[""\u201C](.+?)[""\u201D]\s+u\s+[""\u201C](.+?)[""\u201D]$/i,
      );
      const fillSimple = raw.match(
        /^(upiši|upisi|unesi|unesite)\s+(.+?)\s+u\s+(.+)$/i,
      );
      ...
      if (fillMatch) {
        ...
        if (data?.success) {
          ...
          pushAssistantMessage(
            `❌ Unos u **${target}** nije uspio: ${data?.error || "greška"}`,
          );
        }
        return;
      }
      ...
      if (simpleType) {
        ...
        if (data?.success) {
          ...
          if (fallback?.success) {
            ...
            pushAssistantMessage(
              `❌ Nema fokusiranog polja za unos. Probaj: \`klikni Korisničko ime\` pa onda \`upiši ${value}\``,
            );
          }
        }
        return;
      ...
      // WAIT
      const waitSeconds = raw.match(
        /^(čekaj|cekaj)\s+(\d+)\s*(s|sek|sekundi|sec)?$/i,
      );
      const waitMs = raw.match(/^(čekaj|cekaj)\s+(\d+)\s*ms$/i);
      if (waitSeconds || waitMs) {
        const timeout = waitMs
          ? Number(waitMs[2])
          : Number(waitSeconds?.[2] || 1) * 1000;
        ...
        if (data?.success) {
          ...
          setLastPreviewSummary(
            `Pričekao sam ${timeout}ms i osvježio preview.`,
          );
        } else {
      ...
      if (
        lower.includes("html") ||
        lower.includes("izvuci tekst") ||
        lower.includes("procitaj stranicu") ||
        lower.includes("pročitaj stranicu")
      ) {
        const extracted = await callAgentDirect("playwright", {
          action: "extract",
          timeout: 15000,
        ...
        if (extracted?.success) {
          ...
          pushAssistantMessage(
            `❌ Ne mogu izvući tekst: ${extracted?.error || "nepoznata greška"}`,
          );
        }
        return;
      }
      ...
      pushAssistantMessage(
        'ℹ️ DEV v2 razumije: `idi na https://...`, `klikni Prijava`, `upiši "Marko" u "ime"`, `čekaj 3s`, `screenshot`, `izvuci tekst`.',
      );
    },
    [
      pushAssistantMessage,
  ...
  const executeStudioFlow = useCallback(
  ...
  const handleDevPortalAction = useCallback(
    async (cmd: string) => {
      const raw = cmd.trim();
      const lower = raw.toLowerCase();
      const trimOutput = (value: string, max = 5000) =>
        value.length > max ? value.slice(0, max) + "\n... [skraćeno]" : value;
      const summaryText = (data: any, fallback = "Akcija je izvršena.") => {
        const chunks = [
          typeof data?.message === "string" ? data.message : "",
          typeof data?.summary === "string" ? trimOutput(data.summary) : "",
        ].filter(Boolean);
        return chunks.join("\n\n") || fallback;
      };
      ...
      try {
        if (/^(git status|status)$/i.test(raw)) {
          addLog("info", "→ Git status");
          const data = await callDevControl("git_status");
          addLog("ok", "✓ Git status dohvaćen");
          pushAssistantMessage(
            `### Git status\n\n${summaryText(data, "Git status dohvaćen.")}`,
          );
        } else if (/^(git pull rebase|pull \/ rebase)$/i.test(lower)) {
          addLog("info", "→ Git pull --rebase");
          const data = await callDevControl("git_pull_rebase");
          addLog("ok", "✓ Git pull --rebase prošao");
          pushAssistantMessage(
            `### Git pull / rebase\n\n${summaryText(data, "Git pull --rebase je prošao.")}`,
          );
        } else if (/^(pokreni build|run build|build)$/i.test(raw)) {
          addLog("info", "→ Build");
          const data = await callDevControl("build");
          addLog("ok", "✓ Build je završio");
          pushAssistantMessage(
            `### Build\n\n${summaryText(data, "Build je završen.")}`,
          );
        } else if (/^(git push|push)\b/i.test(raw)) {
          const branchMatch = raw.match(
            /^(?:git push|push)\s+([a-zA-Z0-9._\/-]+)$/i,
          );
          const branch = branchMatch?.[1];
          addLog("info", branch ? `→ Git push ${branch}` : "→ Git push");
          const data = await callDevControl(
            "git_push",
            branch ? { branch } : {},
          );
          addLog("ok", "✓ Git push je prošao");
          pushAssistantMessage(
            `### Git push\n\n${summaryText(data, "Git push je prošao.")}`,
          );
        } else if (/^(git commit|commit)\b/i.test(raw)) {
          const match = raw.match(/^(?:git commit|commit)\s+([\s\S]+)$/i);
          const message =
            match?.[1]?.trim().replace(/^["'`]|["'`]$/g, "") || "";
          if (!message) throw new Error("Commit poruka je obavezna.");
          addLog("info", `→ Git commit: ${message}`);
          const data = await callDevControl("git_commit", { message });
          addLog("ok", "✓ Git commit je prošao");
          pushAssistantMessage(
            `### Git commit\n\n${summaryText(data, `Commit spremljen: ${message}`)}`,
          );
        } else if (/^deploy\b/i.test(raw)) {
          const match = raw.match(/^deploy\s+([\s\S]+)$/i);
          const message =
            match?.[1]?.trim().replace(/^["'`]|["'`]$/g, "") || undefined;
          addLog("info", message ? `→ Deploy: ${message}` : "→ Deploy");
          const data = await callDevControl(
            "deploy",
            message ? { message } : {},
          );
          addLog("ok", "✓ Deploy flow završen");
          pushAssistantMessage(
  ...
  const handlePreviewDescribe = () => {
    void describeCurrentPreview({
      heading: "### Stellan vidi u previewu",
      fallback: "Preview je osvježen, ali nema dovoljno teksta za sažetak.",
    });
  };
  ...
  const handlePreviewWait = () => {
    void waitForPreviewReady(2500).then((result) => {
      if (result?.success) {
        void describeCurrentPreview({
          heading: "### Stellan vidi nakon čekanja",
          fallback:
            "Stranica je čekala učitavanje, ali nema dovoljno teksta za sažetak.",
        });
      }
    });
  };
  ...
  const handleCaptureStep = async () => {
    ...
    try {
      ...
      if (!AGENT_URL) {
        ...
        studioSend(
          `Napravi screenshot trenutne stranice i opiši korak: ${stepDesc || "trenutni prikaz"}`,
        );
        setStepDesc("");
      ...
      const data = await callAgentDirect("screenshot/step", {
        description: desc,
      });
      if (data.success) {
        const step = {
          n: recordedSteps.length + 1,
          url: data.url,
          desc,
  ...
  const handleStartRecording = async () => {
    ...
    if (result?.success) {
      ...
      addLog(
        "warn",
        "Greška pri pokretanju: " + (result?.error || "agent nedostupan"),
      );
  ...
  const handleSaveAction = async () => {
    ...
    const result = await callAgentDirect("record/save", {
      name: recordingName,
    });
    if (result?.success) {
      addLog(
        "ok",
        `Akcija "${recordingName}" spremljena (${result.steps || 0} linija koda)`,
      );
      ...
      addLog(
        "warn",
        "Greška pri spremanju: " + (result?.error || "agent nedostupan"),
      );
    }
    setIsRecording(false);
    setRecordingName("");
  ...
  const handleRunSavedAction = async (name: string) => {
    ...
    if (result?.success) {
      ...
      addLog(
        "warn",
        `Greška pri pokretanju akcije "${name}": ` +
          (result?.error || "agent nedostupan"),
      );
      pushAssistantMessage(
        `❌ Ne mogu pokrenuti akciju **${name}**: ${result?.error || "agent nedostupan"}`,
      );
    }
  };
  ...
  const handleSaveProjectRoot = (value: string) => {
  ...
  const handleStartAgent = async () => {
    ...
    try {
      ...
      await refreshDevOps();
      pushAssistantMessage(
        "ℹ️ Start agent iz web sučelja ne može sam pokrenuti lokalni Windows proces. Pokreni `start_agent.bat` u folderu `docs/agent-server`, pa zatim klikni `Check agent` ili `Refresh status`.",
      );
      addLog(
        "info",
      ...
      setInput(
        "Pokreni start_agent.bat iz foldera: D:\\1 PROJEKT APLIKACIJA LOKALNO\\1 PROJEKT APLIKACIJA LOKALNO claude\\STELLAN-GIT\\docs\\agent-server",
      );
        ...
        <div className="relative h-[98vh] w-[99vw] overflow-hidden rounded-2xl border border-white/[0.06] bg-[hsl(220,15%,7%)] shadow-2xl">
          <DevPanel
            ...
            modelBadge={
              ...
              )?.badge || MODEL_BADGES[selectedModel]
            }
            ...
            onDeleteStep={(stepId) =>
              setDevSteps((prev) => prev.filter((step) => step.id !== stepId))
            }
            onSelectStep={(step) =>
              addLog("info", `Odabran korak: ${step.label}`)
            }
            ...
            onRunSavedAction={(name) => {
              void handleRunSavedAction(name);
            }}
            onRefreshActions={() => {
              void handleRefreshActions();
            }}
            ...
            onPortalAction={(cmd) => {
              void handleDevPortalAction(cmd);
            }}
  ...
  Mozak V2
          ...
          <div
            className={cn(
              ...
              dragOver && "ring-2 ring-emerald-400/40 ring-inset",
            )}
              ...
              <div className="flex flex-col items-center justify-center h-full px-6 gap-6">
                ...
                <div className="text-center space-y-1.5">
                  ...
                  <p className="text-white/35 text-base max-w-lg leading-relaxed">
                    Pamtim razgovore, koristim znanje iz mozga na Google Driveu
                    i pretražujem internet.
                  </p>
                </div>
                {/* Suggested questions */}
                <div className="grid grid-cols-2 gap-2 w-full max-w-lg">
                  {[
                    {
                      icon: "🔍",
                      text: "Pretraži SDGE portal",
                      prompt: "Pretraži SDGE portal za najnovije zahtjeve",
                    },
                    {
                      icon: "📁",
                      text: "Pretraži Google Drive",
                      prompt: "Pretraži firmeni Google Drive za dokumente",
                    },
                    {
                      icon: "📧",
                      text: "Pretraži Gmail",
                      prompt: "Pretraži Gmail inbox za nepročitane poruke",
                    },
                    {
                      icon: "📊",
                      text: "Status projekata",
                      prompt:
                        "Pokaži mi status aktivnih projekata u GeoTerra aplikaciji",
                    },
                    {
                      icon: "🧾",
                      text: "Provjeri račune",
                      prompt: "Pretraži Solo fakturiranje za posljednje račune",
                    },
                    {
                      icon: "🔎",
                      text: "Provjeri OIB",
                      prompt: "Provjeri OIB za osobu ili tvrtku",
                    },
                  ].map((s) => (
                    <button
                      key={s.text}
          ...
          <div className="p-3 border-t border-white/[0.06]">
                ...
                <div className="flex gap-2 flex-wrap px-3 pt-3 pb-1">
                  {pendingImages.map((img, i) => (
                    <div key={`img-${i}`} className="relative group">
                      ...
                      <div className="text-[9px] text-white/30 mt-0.5 truncate w-16">
                        {img.name}
                      </div>
                    </div>
                  ))}
                  {pendingFiles.map((file, i) => {
                    ...
                    return (
                      <div key={`file-${i}`} className="relative group">
                        ...
                        <div className="text-[9px] text-white/30 mt-0.5 truncate w-20">
                          {file.name}
                        </div>
                      </div>
                    );
            ...
            <div className="flex items-center justify-center gap-3 mt-1.5">
              ...
              <input
                ...
                value={budgetEur}
                onChange={(e) => {
                  setBudgetEur(e.target.value);
                  localStorage.setItem("stellan_budget", e.target.value);
                }}
                placeholder="€"
              ...
              <button
                onClick={() => {
                  setSpentEur(0);
                  localStorage.setItem("stellan_spent", "0");
                }}
