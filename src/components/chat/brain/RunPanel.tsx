...
const RunPanel = ({ nodes, connections, runStatuses, isRunning, onRun, onStop, onReset, onPreview }: Props) => {
  ...
  return (
    <motion.div
      ...
      style={{ background: "rgba(6,18,16,0.92)", backdropFilter: "blur(16px)", minWidth: 420 }}
    >
      {/* Header */}
      <div className="flex items-center justify-between px-4 py-2.5 border-b border-white/[0.06]">
        <div className="flex items-center gap-3">
          <span className="text-xs font-semibold text-white/80">Flow Runner</span>
          <div className="flex items-center gap-1.5 text-[10px] text-emerald-100/30">
        ...
        <div className="flex items-center gap-1.5">
          ...
          {totalTime > 0 && (
            <span className="flex items-center gap-1 text-[9px] text-emerald-100/30">
              <Clock className="w-2.5 h-2.5" /> {(totalTime / 1000).toFixed(1)}s
            </span>
          )}
          <button onClick={() => setExpanded(!expanded)} className="text-emerald-100/30 hover:text-white/72 transition-colors ml-1">
      ...
      <div className="flex items-center gap-2 px-4 py-2.5">
        ...
        <button onClick={onPreview}
          className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-white/[0.04] text-emerald-100/55 border border-white/[0.06] hover:bg-white/[0.08] hover:text-emerald-50 transition-colors text-xs">
            ...
            .map(status => {
              ...
              return (
                <div key={status.nodeId} className="flex items-center gap-2.5 px-4 py-1.5 border-b border-white/[0.03] last:border-0">
                  ...
                  <span className="text-[10px] text-white/72 flex-1 truncate">{node?.label || status.nodeId}</span>
                  {status.duration && (
                    <span className="text-[9px] text-emerald-100/28">{status.duration}ms</span>
                  )}
                  {status.output && (
