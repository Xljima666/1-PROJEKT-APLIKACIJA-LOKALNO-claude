...
const Minimap = ({
  ...
  return (
    <div
      className="absolute bottom-4 right-4 z-30 rounded-xl overflow-hidden border border-white/[0.08]"
      ...
      >
        ...
        {nodes.map((n) => (
          <rect
            ...
            rx={2}
            fill={MINIMAP_COLORS[n.category] || "#888"}
            fillOpacity={0.5}
          />
        ))}
        <rect
...
const ZoomControls = ({
  ...
  <div
    className="absolute bottom-20 left-4 z-30 flex flex-col gap-1 rounded-xl overflow-hidden border border-white/[0.08]"
    style={{ background: "rgba(10,8,20,0.85)", backdropFilter: "blur(12px)" }}
  >
    <button
      onClick={onIn}
      className="w-9 h-9 flex items-center justify-center text-white/50 hover:text-white hover:bg-white/[0.06] transition-colors"
    >
      <Plus className="w-4 h-4" />
    </button>
    <div className="text-[9px] text-white/30 text-center py-0.5 border-y border-white/[0.06]">
      {Math.round(zoom * 100)}%
    </div>
    <button
      onClick={onOut}
    ...
    <button
      onClick={onFit}
      className="w-9 h-9 flex items-center justify-center text-white/50 hover:text-white hover:bg-white/[0.06] transition-colors border-t border-white/[0.06]"
    >
  ...
  <div className="space-y-1.5">
    ...
    {multiline ? (
      <textarea
        ...
        className="w-full min-h-[72px] rounded-lg bg-white/[0.04] border border-white/[0.08] px-3 py-2 text-xs text-white/85 placeholder:text-white/20 outline-none focus:border-white/[0.16] resize-y"
      />
    ) : (
      <input
        ...
        placeholder={placeholder}
        onChange={(e) => onChange(e.target.value)}
        className="w-full h-9 rounded-lg bg-white/[0.04] border border-white/[0.08] px-3 text-xs text-white/85 placeholder:text-white/20 outline-none focus:border-white/[0.16]"
      />
    )}
  </div>
);
  ...
  return (
    ...
    >
      <div className="absolute inset-0 pointer-events-none overflow-hidden">
        <div
          className="absolute top-[15%] left-[20%] w-[500px] h-[500px] rounded-full opacity-[0.05]"
          style={{ background: "radial-gradient(circle, rgba(167,139,250,1), transparent 70%)" }}
        />
        <div
          className="absolute top-[50%] right-[15%] w-[400px] h-[400px] rounded-full opacity-[0.04]"
          style={{ background: "radial-gradient(circle, rgba(56,189,248,1), transparent 70%)" }}
        />
        <div
          className="absolute bottom-[10%] left-[40%] w-[350px] h-[350px] rounded-full opacity-[0.04]"
      ...
      <div className="relative z-10 flex items-center justify-between px-5 py-2.5 border-b border-white/[0.06]">
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            <div
              ...
              style={{ background: "linear-gradient(135deg, rgba(167,139,250,0.3), rgba(139,92,246,0.15))" }}
            >
              <Brain
            ...
            <input
              ...
              onChange={(e) => actions.setFlowName(e.target.value)}
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="relative">
            <button
              onClick={() => actions.setShowFlowMenu(!state.showFlowMenu)}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-white/[0.06] hover:bg-white/[0.1] border border-white/[0.08] text-white/50 hover:text-white transition-all text-xs"
            >
            ...
            {state.showFlowMenu && (
              <>
                ...
                <div
                  className="absolute right-0 top-full mt-1 z-50 w-56 rounded-xl border border-white/[0.08] overflow-hidden"
                ...
                >
                  {state.savedFlows.length === 0 ? (
                    ...
                    state.savedFlows.map((flow) => (
                      <button
                        key={flow.id}
                        onClick={() => actions.loadFlow(flow.id)}
      ...
      <div className="flex-1 flex min-h-0 relative z-10">
        ...
        <div
          ...
          className={cn(
            ...
            state.isPanning ? "cursor-grabbing" : state.drawingConn ? "cursor-crosshair" : "cursor-grab"
          )}
            ...
            <svg className="absolute inset-0 pointer-events-none" width={CANVAS_W} height={CANVAS_H} style={{ zIndex: 1 }}>
              {state.connectionPaths.map((cp, i) => (
                ...
                >
                  ...
                  <AnimatedConnection
                    d={cp.d}
                    color={state.selectedConnection === cp.id ? "#fff" : cp.color}
                    index={i}
                    isActive={state.activeNodes.includes(cp.fromNode) || state.activeNodes.includes(cp.toNode)}
                  />
                </g>
              ))}
            ...
            >
              ...
              {label === "input" && (
                <InspectorField
                  ...
                  placeholder="Unesi ulaznu vrijednost..."
                  onChange={(v) => actions.updateNodeConfig(selected.id, "value", v)}
                  multiline
                />
              )}
              ...
              {label === "browser open" && (
                <>
                  <InspectorField
                    ...
                    onChange={(v) => actions.updateNodeConfig(selected.id, "url", v)}
                  />
                  <InspectorField
                    label="Timeout (ms)"
                    value={selectedConfig.timeout ?? 45000}
                    numeric
                    onChange={(v) => actions.updateNodeConfig(selected.id, "timeout", Number(v || 0))}
                  />
                </>
              )}
              ...
              {label === "click" && (
                <>
                  <InspectorField
                    label="Selector"
                    value={selectedConfig.selector ?? ""}
                    placeholder='text=Prijava ili button:has-text("Prijava")'
                    onChange={(v) => actions.updateNodeConfig(selected.id, "selector", v)}
                  />
                  <InspectorField
                    label="Timeout (ms)"
                    value={selectedConfig.timeout ?? 20000}
                    numeric
                    onChange={(v) => actions.updateNodeConfig(selected.id, "timeout", Number(v || 0))}
                  />
                </>
              ...
              {label === "fill input" && (
                <>
                  ...
                  <InspectorField
                    label="Value"
                    value={selectedConfig.value ?? ""}
                    placeholder="Vrijednost za upis"
                    onChange={(v) => actions.updateNodeConfig(selected.id, "value", v)}
                  />
                  <InspectorField
                    label="Timeout (ms)"
              ...
              {label === "extract text" && (
                <InspectorField
                  label="Timeout (ms)"
                  value={selectedConfig.timeout ?? 15000}
                  numeric
                  onChange={(v) => actions.updateNodeConfig(selected.id, "timeout", Number(v || 0))}
                />
              )}
              ...
              {label === "screenshot" && (
                <>
                  <InspectorField
                    label="Full page"
                    value={selectedConfig.full_page ? "true" : "false"}
                    placeholder="true / false"
                    onChange={(v) => actions.updateNodeConfig(selected.id, "full_page", v === "true")}
                  />
                  <InspectorField
                    label="Timeout (ms)"
                    value={selectedConfig.timeout ?? 15000}
                ...
                {selected.ports.map((port) => (
                  <div key={port.id} className="flex items-center gap-2 text-xs text-white/60">
                    <div
                      ...
                      style={{
                        backgroundColor: PORT_COLORS[port.color],
                        boxShadow: `0 0 4px ${PORT_COLORS[port.color]}40`,
                      }}
                    />
              ...
              <div className="space-y-2">
                ...
                {state.connections
                  .filter((c) => c.fromNode === selected.id || c.toNode === selected.id)
                  .map((c, i) => {
              ...
              {state.runStatuses[selected.id] && state.runStatuses[selected.id].status !== "idle" && (
                <div className="pt-3 border-t border-white/[0.06] space-y-1">
                  ...
                  >
                    {state.runStatuses[selected.id].status}
                  </p>

                  {state.runStatuses[selected.id].duration && (
                    <p className="text-[10px] text-white/20">{state.runStatuses[selected.id].duration}ms</p>
                  )}

                  {state.runStatuses[selected.id].error && (
                    <p className="text-[10px] text-red-400/60">{state.runStatuses[selected.id].error}</p>
                  )}

                  {state.lastRunOutputs[selected.id]?.output !== undefined && (
                    <div className="pt-2">
                      <p className="text-[10px] font-semibold text-white/30 uppercase tracking-wider mb-1">Output</p>
                      <pre className="text-[10px] text-white/65 whitespace-pre-wrap break-words rounded-lg bg-white/[0.03] border border-white/[0.06] p-2">
                        {typeof state.lastRunOutputs[selected.id].output === "string"
                          ? state.lastRunOutputs[selected.id].output
                          : JSON.stringify(state.lastRunOutputs[selected.id].output, null, 2)}
                      </pre>
                    </div>
                  )}
                </div>
